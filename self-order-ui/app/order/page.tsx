            ...
            <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-6 py-10 lg:grid lg:grid-cols-[1.2fr_0.8fr]">
                <section className="flex flex-col gap-6">
                    ...
                    {!restaurantCode || !tableId ? (
                        <div className="glass-panel rounded-2xl p-6 text-sm text-[#6b5f57]">
                            Add <strong>restaurantCode</strong> and <strong>tableId</strong> in the URL to start
                            ordering.
                        </div>
                    ) : menuQuery.isLoading ? (
                        <div className="glass-panel rounded-2xl p-6 text-sm text-[#6b5f57]">Loading menu...</div>
                    ) : menuQuery.error ? (
                                                ...
                                                <div className="flex items-start justify-between gap-2">
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-[#6b5f57]">
                                                            {item.description || "Seasonal favorite"}
                                                        </p>
                                                    </div>
                ...
                <aside className="flex flex-col gap-4 lg:sticky lg:top-6 lg:self-start">
                    <div className="glass-panel rounded-2xl p-5">
                        ...
                        {cartItems.length === 0 ? (
                            <p className="mt-3 text-sm text-[#6b5f57]">Add items to begin your order.</p>
                        ) : (
                            <div className="mt-4 flex flex-col gap-3">
                                ...
                                <div className="flex items-center justify-between border-t border-[#e6d9cd] pt-3 text-sm">
                                    <span className="text-[#6b5f57]">Total</span>
                    ...
                    <div className="glass-panel rounded-2xl p-5">
                        <h2 className="font-display text-xl">Service requests</h2>
                        <p className="mt-2 text-xs text-[#6b5f57]">Need help? Send a quick signal to the staff.</p>
                        ...
                        {callMutation.error ? (
                            <p className="mt-2 text-xs text-red-700">
                                {callMutation.error instanceof Error ? callMutation.error.message : "Request failed"}
                            </p>
                        ) : null}
                    </div>
                    ...
                    <div className="glass-panel rounded-2xl p-5">
                        ...
                        {latestOrder ? (
                            <div className="mt-3 flex flex-col gap-2 text-sm">
                                ...
                                <p className="text-xs text-[#6b5f57]">
                                    Order #{latestOrder.id} • {latestOrder.items.length} items
                                </p>
                            </div>
                        ) : (
                            <p className="mt-3 text-xs text-[#6b5f57]">Place an order to see live updates here.</p>
                        )}
                            ...
                            <div className="mt-4 max-h-40 space-y-2 overflow-auto text-xs text-[#6b5f57]">
                                {orderUpdates.slice(0, 5).map((order) => (
                                    <div key={`${order.id}-${order.updatedAt}`}>
                                        #{order.id} updated to {statusLabels[order.status]}
                                    </div>
                                ))}
                            </div>
                        ) : null}
